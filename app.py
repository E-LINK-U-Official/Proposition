...
st.markdown("""
    ...
    .sector-card {
        ...
        background-color: #0e1117; margin-bottom: 10px; min-height: 220px;
    }
    </style>
...
# Diccionario Maestro de Textos (Híbrido)
text = {
    "English": {
        ...
        "r3_t": "🌐 Phase 3: Total Interop", "r3_d": "Focus: Universal EU Citizenry. Card as the permanent offline 'Anchor'.",
    ...
    "Español": {
        ...
        "chip_info": "La antena está bloqueada físicamente hasta detectar una huella dactilar viva. Ningún dato biométrico sale de la tarjeta.",
        "s1_t": "🟢 Finanzas (Verde)", "s1_p": "Economía C2C Offline. El comercio continúa durante apagones o hackeos bancarios.",
        "s2_t": "🔴 Salud (Rojo)", "s2_p": "Datos críticos accesibles por tarjeta en emergencias o zonas sin señal.",
        "s3_t": "🔵 Identidad (Azul)", "s3_p": "Identidad Autosoberana. Llave Maestra offline para seguridad fronteriza y eIDAS 2.0.",
        "pillar_h": "🛡️ Pilares Estratégicos: Privacidad y Resiliencia",
        "p1_t": "Privacidad Zero-Knowledge", "p1_d": "Verificación sin exponer datos privados. Soberanía por diseño (ZKP).",
        "p2_t": "ROI Instantáneo", "p2_d": "Recuperación proyectada de 459B€/año. Costes amortizados en 30 días.",
        "p3_t": "Resiliencia Híbrida", "p3_d": "Seguridad total: Digital + Tarjeta Física Biométrica para ciberataques.",
        ...
        "r2_t": "🚄 Fase 2: Corredores UE", "r2_d": "Foco: Trabajadores Móviles. Despliegue híbrido para identidad ferroviaria.",
        ...
        "legal_d": "La arquitectura e-link-u y el protocolo Umatter son activos propietarios de Lia Ariadna Ruiz Ben. Conforme a RGPD y estándares EUDI."
    }
}[lang]
...
try:
    ...
    if not df.empty:
        ...
        with pa:
            st.subheader(text["p1_t"])
            st.write(text["p1_d"])
        with pb:
            st.subheader(text["p2_t"])
            st.write(text["p2_d"])
        with pc:
            st.subheader(text["p3_t"])
            st.write(text["p3_d"])
